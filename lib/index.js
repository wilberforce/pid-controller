...
var PID = function(Input, Setpoint, Kp, Ki, Kd, ControllerDirection) {
    if (ControllerDirection == 0 || 'direct' || 'Direct' || 'DIRECT') {
        this.setControllerDirection(0);
    }
    else if (ControllerDirection == 1 || 'reverse' || 'Reverse' || 'REVERSE') {
        this.setControllerDirection(1);
    }
    else {
        throw new Error("Incorrect Controller Direction");
    }
...
PID.prototype.setTunings = function(Kp, Ki, Kd) {
    ...
    if (this.controllerDirection == this.REVERSE) {
        this.kp = (0 - this.kp);
        this.ki = (0 - this.ki);
        this.kd = (0 - this.kd);
    }
